...
Gagal jantung merupakan salah satu penyakit kardiovaskular kronis dengan angka morbiditas dan mortalitas yang tinggi secara global, sehingga menjadi beban substansial bagi sistem kesehatan \citep{savarese2022global}. Studi Global Burden of Disease melaporkan bahwa Penyakit jantung koroner atau ischemic heart disease (IHD) menyumbang sekitar 9,1 juta kematian dan sekitar 180 juta disability-adjusted life years (DALYs) pada tahun 2019, dengan jumlah kematian yang terus meningkat sejak 1990 meskipun laju mortalitas terseragam usia cenderung menurun \citep{wang2021global}. Beban tersebut sangat menonjol di kawasan Asia dan negara berpendapatan rendah - menengah, serta penyakit kardiovaskular telah menjadi penyebab kematian utama dan mayoritas kematian akibat IHD kini bergeser dari negara maju ke negara berkembang \citep{zhao2021epidemiological}. Kondisi ini tidak hanya mencerminkan tingginya kebutuhan rawat inap dan intervensi kardiovaskular, tetapi juga menurunkan kualitas hidup jangka panjang serta menambah tekanan ekonomi pada pasien, keluarga, dan sistem pelayanan kesehatan, sehingga penting untuk adanya strategi deteksi dan stratifikasi risiko penyakit gagal jantung yang lebih dini dan dapat digeneralisasi lintas populasi.
...
```{r categorical-analysis, eval=TRUE, warning=FALSE, message=FALSE, comment=NA, fig.align='center', fig.show='hold', out.width='45%', fig.asp=0.6}
...
p_chest <- ggplot(df, aes(x = ChestPainType, fill = factor(HeartDisease))) +
  ...
  labs(
    ...
    subtitle = "ASY = Asymptomatic, ATA = Atypical Angina\nNAP = Non-Anginal Pain, TA = Typical Angina",
    x = "Tipe Nyeri Dada",
    y = "Proporsi"
  ) + theme_minimal() +
  theme(
...
p_slope <- ggplot(df, aes(x = ST_Slope, fill = factor(HeartDisease))) +
  ...
  labs(
    ...
    subtitle = "Up = Upsloping, Flat = Flat\nDown = Downsloping",
    x = "ST Slope Pattern",
    y = "Proporsi"
  ) + theme_minimal() +
  theme(
...
Sebelum dilakukan seleksi fitur, terlebih dahulu dibangun model baseline yang dilatih pada **seluruh fitur**, seperti memperoleh tolok ukur awal performa model, mengidentifikasi fitur yang berkontribusi signifikan (melalui *variable importance*), dan memahami pola keterkaitan umum dalam data.
...
Perbandingan performa CV menunjukkan bahwa Model Final (9 fitur) memiliki akurasi CV 0.8527 dan Kappa CV 0.7012, yang sedikit lebih rendah dibandingkan Model Baseline (11 fitur) dengan akurasi CV 0.8568 dan Kappa CV 0.7099. Penurunan performa yang sangat minimal ini (<0.005 untuk Akurasi dan Kappa) mengindikasikan bahwa fitur yang dihapus (FastingBS dan RestingECG) memang merupakan noise atau fitur dengan kontribusi prediktif yang minimal, sehingga model akhir berhasil disempurnakan dan disederhanakan (pengurangan 18% fitur) tanpa kehilangan kemampuan generalisasi yang signifikan.
...
- **False Negative (FN):** Pasien sakit diprediksi sehat - **PALING BERBAHAYA**
...
#### Imputasi (Missing Value Handling)

- **Masalah:**  
  ~18% data memiliki `Cholesterol = 0`

- **Solusi:**  
  Median imputation berdasarkan *training set*
...
* **Train-only Calculation:** 

  Critical untuk menghindari data leakage
...
* **Untuk Random Forest:** 
  ...
  Dampak minimal (tree-based methods scale-invariant)

* **Best practice:** 

  jika pada tahap selanjutnya digunakan algoritma lain (misalnya *logistic regression*, SVM, atau *neural network*), data sudah berada dalam skala yang seragam
  
* **Interpretasi:** 

  Koefisien/importance lebih mudah dibandingkan jika semua fitur pada skala yang sama
...
* *Bagaimana membangun model klasifikasi untuk memprediksi penyakit jantung?*
...
* *Bagaimana menangani masalah kualitas data seperti nilai yang tidak logis?*
...
* *Apakah seleksi fitur dapat menyederhanakan model dan meningkatkan kinerjanya?*
